...
import { formatDateBR, convertBRToISO, convertISOToBR, isValidDateBR } from '@/utils/dateFormatters';
...
const Classes = () => {
  ...
  // Estados para formatação de datas brasileiras
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (editingClass) {
        ...
        // Criar nova turma - incluir campos de datas e plano
        const insertData = {
          nome: data.nome.trim(),
          idioma: data.idioma,
          dias_da_semana: data.dias_da_semana,
  ...
  const openEditDialog = (classItem: Class) => {
    ...
    // Sincronizar campos de data brasileira
    setDataInicioBR(classItem.data_inicio ? convertISOToBR(classItem.data_inicio) : '');
    setDataFimBR(classItem.data_fim ? convertISOToBR(classItem.data_fim) : '');
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="mb-4">
                  <div className="flex justify-center">
                    <div className="w-full max-w-xs">
                      <Label htmlFor="total_aulas" className="text-sm font-medium text-gray-700 block text-center mb-2">
                        Total de Aulas
                      </Label>
                      <Input
                        id="total_aulas"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Data de Início */}
                  <div>
                    <Label htmlFor="data_inicio" className="text-sm font-medium text-gray-700 block text-center mb-2">
                      Data de Início
                    </Label>
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setDataInicioBR(formatted);
                          
                          // Converter para ISO e atualizar o formulário se a data estiver completa e válida
                          if (formatted.length === 10 && isValidDateBR(formatted)) {
                            const isoDate = convertBRToISO(formatted);
                            setValue('data_inicio', isoDate);
                          } else {
                            setValue('data_inicio', '');
                          }
                        }}
                        onClick={() => {
                          const dateInput = document.querySelector('input[type="date"][data-field="data_inicio"]') as HTMLInputElement;
                          if (dateInput) dateInput.showPicker();
                        }}
                        className="text-center pr-10 cursor-pointer"
                      />
                      <input
                        type="date"
                        ...
                        onChange={(e) => {
                          setValue('data_inicio', e.target.value);
                          if (e.target.value) {
                            setDataInicioBR(convertISOToBR(e.target.value));
                          } else {
                            setDataInicioBR('');
                          }
                        }}
                      ...
                      />
                      <Calendar className="absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400 z-20 pointer-events-none" />
                    </div>
                    {errors.data_inicio && (
                      <div className="mt-1 flex items-center justify-center gap-1">
                        <AlertTriangle className="h-4 w-4 text-red-500" />
                  ...
                  {/* Data de Fim */}
                  <div>
                    <Label htmlFor="data_fim" className="text-sm font-medium text-gray-700 block text-center mb-2">
                      Data de Fim
                    </Label>
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) => {
                          const formatted = formatDateBR(e.target.value);
                          setDataFimBR(formatted);
                          
                          // Converter para ISO e atualizar o formulário se a data estiver completa e válida
                          if (formatted.length === 10 && isValidDateBR(formatted)) {
                            const isoDate = convertBRToISO(formatted);
                            setValue('data_fim', isoDate);
                          } else {
                            setValue('data_fim', '');
                          }
                        }}
                        onClick={() => {
                          const dateInput = document.querySelector('input[type="date"][data-field="data_fim"]') as HTMLInputElement;
                          if (dateInput) dateInput.showPicker();
                        }}
                        className="text-center pr-10 cursor-pointer"
                      />
                      <input
                        ...
                        onChange={(e) => {
                          setValue('data_fim', e.target.value);
                          if (e.target.value) {
                            setDataFimBR(convertISOToBR(e.target.value));
                          } else {
                            setDataFimBR('');
                          }
                        }}
                ...
                <div className="flex justify-center mt-4">
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const totalAulas = getValues('total_aulas');
                      const diasSemana = selectedDays;
                      const horario = getValues('horario');

                      if (!dataInicio) {
                        toast({
                          title: "Campo obrigatório",
                          description: "Por favor, preencha a data de início primeiro.",
                          variant: "destructive",
                        });
                        return;
                      }
                      ...
                      if (!totalAulas || totalAulas <= 0) {
                        toast({
                          title: "Campo obrigatório",
                          description: "Por favor, preencha o total de aulas primeiro.",
                          variant: "destructive",
                        });
                        return;
                      }
                      ...
                      if (diasSemana.length === 0) {
                        toast({
                          title: "Campo obrigatório",
                          description: "Por favor, selecione os dias da semana primeiro.",
                          variant: "destructive",
                        });
                        return;
                      }
                      ...
                      if (!horario) {
                        toast({
                          title: "Campo obrigatório",
                          description: "Por favor, preencha o horário primeiro.",
                          variant: "destructive",
                        });
                        return;
                      }
                      ...
                      try {
                        const dataFimCalculada = calculateEndDateWithHolidays(
                          dataInicio,
                          totalAulas,
                          diasSemana,
                          horario
                        ...
                        setValue('data_fim', dataFimCalculada.endDate);
                        setDataFimBR(convertISOToBR(dataFimCalculada.endDate));
                        
                        toast({
                          title: "Data calculada com sucesso!",
                          description: `Data de fim sugerida: ${formatDateForDisplay(dataFimCalculada.endDate)}`,
                        });
                      } catch (error) {
                        console.error('Erro ao calcular data de fim:', error);
                        toast({
                          title: "Erro no cálculo",
                          description: "Não foi possível calcular a data de fim. Verifique os dados informados.",
                          variant: "destructive",
                        });
                      }
                    }}
                 ...
                 <div>
                   ...
                   <div className="relative">
                     <Input
                       ...
                       onClick={() => {
                         const dateInput = document.querySelector('input[type="date"][data-field="modal_data_inicio"]') as HTMLInputElement;
                         if (dateInput) dateInput.showPicker();
                       }}
                       ...
                       <Button
                         ...
                         onClick={() => {
                           ...
                           if (!horario || !horario.includes(' - ')) {
                             toast({
                               title: "Campo obrigatório",
                               description: "Por favor, preencha o horário completo primeiro.",
                               variant: "destructive",
                             });
                             return;
                           }
                           ...
                           try {
                             ...
                             setValue('data_fim', result.endDate);
                             setDataFimBR(convertISOToBR(result.endDate));
                             setCalculatedEndDate(result.endDate);
                             setDetectedHolidays(result.holidaysFound);

                             toast({
                               title: "Data sugerida",
                               description: `Data de fim calculada: ${formatDateForDisplay(result.endDate)}${result.holidaysFound.length > 0 ? ` (${result.holidaysFound.length} feriado(s) considerado(s))` : ''}`,
                             });
                           } catch (error) {
                             console.error('Erro ao calcular data de fim:', error);
                             toast({
                         ...
                         className="h-7 px-2 flex items-center gap-1 bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"
                         title="Calcular data de fim automaticamente"
                       >
                         <CalendarDays className="h-3 w-3" />
                         <span className="text-xs">Sugerir Datas</span>
                       </Button>
                     </div>
                   </div>
                   <div className="relative">
                     <Input
                        ...
                        onClick={() => {
                          const dateInput = document.querySelector('input[type="date"][data-field="modal_data_fim"]') as HTMLInputElement;
                          if (dateInput) dateInput.showPicker();
                        }}
                        className="text-center pr-10 cursor-pointer"
                      />
